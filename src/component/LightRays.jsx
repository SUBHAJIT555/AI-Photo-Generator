...
function LightRays({
  ...
  useEffect(() => {
    ...
    const checkSize = () => {
      ...
      setIsReady(clientWidth > 1 && clientHeight > 1);
    };
...
void main() {
  ...
  gl_FragColor = color;
      ...
      const placementObserver = new ResizeObserver(updatePlacement);
      placementObserver.observe(containerRef.current);
      updatePlacement();
      animationIdRef.current = requestAnimationFrame(loop);
  ...
  return (
    <div
      ref={containerRef}
      className={`light-rays pointer-events-none absolute inset-0 h-full w-full overflow-hidden ${className}`.trim()}
    />
  );
}
